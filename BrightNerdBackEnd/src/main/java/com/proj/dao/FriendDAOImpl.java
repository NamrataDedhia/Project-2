...
public class FriendDAOImpl implements FriendDAO{
	...
	private SessionFactory sessionFactory;
		...
		public List<User> getSuggestedUsers(String email) {
			...
			String queryString="select * from user_bn where email in"
					+ "( select email from user_bn where email!=? "
					+ " minus "
					+ " (select fromId_email from friend_bn where toId_email=? "
					+ "   union "
					+ "  select toId_email from friend_bn where fromId_email=?))";
